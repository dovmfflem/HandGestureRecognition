...
public class LifeGesture extends JFrame
{
	...
	public static void main(String[] args) throws IOException, InterruptedException
	{
		...
		final String portName = "/dev/ttyUSB0"; //"COM7" "COM6" - notebook 
...
//			}
//			else if(setting == 0) //남성용이면, -> 여성용이랑 다른 학습 모델을 사용할 경우에만.
//			{
//				classsetting = 1;
...
//					System.out.printf("%d:%d ", k, data[k-1]);
			...
			updateIMU((int)result[allOfSensors+setting-6], (int)result[allOfSensors+setting-5], (int)result[allOfSensors+setting-4], (int)result[allOfSensors+setting-3], (int)result[allOfSensors+setting-2], (int)result[allOfSensors+setting-1]);
			if(err == 1)
			{
...
//			System.out.printf("%d %d %d %d %d %d \n", (int)result[allOfSensors+setting-6], (int)result[allOfSensors+setting-5], (int)result[allOfSensors+setting-4], (int)result[allOfSensors+setting-3], (int)result[allOfSensors+setting-2], (int)result[allOfSensors+setting-1]);
			...
			for(i=6; i>0; i--)
			{
				IMUsum += (int)Math.abs(result[allOfSensors+setting-i]-previous_result[allOfSensors+setting-i]);
...
//			if(setting == 1)  //남성용이면, -> 여성용이랑 다른 학습 모델을 사용할 경우에만.
//			{
...
//			if (none_count > 50)
//				updateGesture(0);
			if (previous_prediction==prediction && predict_count>10 && Math.abs(result[allOfSensors+setting-6]) > 500) //9-uart 7-serial
			{
...
//					update_count = 0;
			...
			System.out.printf("\t%.1f:%.1f\t", svm_data[index6to9], Math.abs(result[allOfSensors+setting-3]));
...
//		int m = dimension; // input data dimension
	...
	public LifeGesture()
	{
		...
		btnReset.setBounds(298, 12, 109, 39);
...
//			int dataLength = (sensorData.length < numOfPoints)?(sensorData.length):(numOfPoints);
			...
			for(int i = 0; i <numOfFingers; ++i)
			{
				...
				{
//					System.out.printf("%d %d : %s \n", i, j, Integer.toString(sensorData[k]));
