...
//		System.out.println(sensingBuffer);
		String dataBody = null;
		int starterIndex = sensingBuffer.indexOf(starter); // 嶺뚳퐙爰귟떋�먯�ゅ뜝占� S0
		int finisherIndex = sensingBuffer.indexOf(finisher, starterIndex+1) - 1; // single
		if(starterIndex < 0 && finisherIndex < 0)
		{
			starterIndex = sensingBuffer.indexOf(l_starter); // 嶺뚳퐙爰귟떋�먯�ゅ뜝占� S0
			...
			if(starterIndex < 0 && finisherIndex < 0)
			{
				starterIndex = sensingBuffer.indexOf(r_starter); // 嶺뚳퐙爰귟떋�먯�ゅ뜝占� S0
...
//		System.out.printf("%d\t%d\t", sensingBuffer.length(), dataBody.length());
		...
		if(sensingBuffer.length() > packetSize*1000){ // 占쎈듇占쎌읅占쎈쭆 data set占쎌뵠 1000揶쏉옙 占쎌뵠占쎄맒占쎌뵠筌롳옙
...
/*// 占쎈립 占쎈�筌랃옙 占쎈염野껉퀣�뻻 
		...
		for(i=0; i<5; i++)
		{
			...
			{
				data = dataBody.substring(sensorPos, dataBody.indexOf("\t",sensorPos+1));
				SensorValue[sensorNum] = Double.parseDouble(data); // flexible - (double)Integer.parseInt(strHexValue, 16)); //16筌욊쑴�땾�몴占� 10筌욊쑴�땾嚥∽옙
...
//			sensorNum ++;
		...
		data = dataBody.substring(sensorPos, dataBody.indexOf("\t",sensorPos+1)); // S占쎌뵬 占쎈르占쎈뮉 NO.
		sensorPos = sensorPos + data.length() + 1; // L or R delete 
		for(j=0; j<numOfGyro-1; j++)		//IMU sensor datas
		{
			data = dataBody.substring(sensorPos, dataBody.indexOf("\t",sensorPos+1));
			SensorValue[numOfPressure+numOfStrech+j] = Double.parseDouble(data); // flexible - (double)Integer.parseInt(strHexValue, 16)); //16筌욊쑴�땾�몴占� 10筌욊쑴�땾嚥∽옙
...
//			System.out.println(split_dataBody[k]);
			if(split_dataBody[k].indexOf("\t",sensorPos+1) < 0) //�삁�쇅泥섎━
			{
				k ++;
				sensorPos = 0;
				continue;				
...
//			System.out.println(numOfPressure+i + " - " + SensorValue[numOfPressure+i]); // print v1114
		...
		while(i<6)
		{
			data = split_dataBody[k].substring(sensorPos, split_dataBody[k].indexOf("\t",sensorPos+1)); // S占쎌뵬 占쎈르占쎈뮉 NO.
			sensorPos = sensorPos + data.length() + 1; // L or R delete 
			if(!data.equals(finger[i]))
			{
				k ++;
...
//		String finisherArr[] = {"S1", "S2", "S3", "S0"};
		...
		String dataBody = null;
		int starterIndex = sensingBuffer.indexOf(starter); // 嶺뚳퐙爰귟떋�먯�ゅ뜝占� S0
		int finisherIndex = sensingBuffer.indexOf(starter, starterIndex+1) - 1; // �뜝�럥�걗�뵓怨뺣쐠占쎈윯 S0
		final int startPosition = 0;
		final int initialValue = 0;
...
//		String finisherArr[] = {"S1", "S2", "S3", "S0"};
		...
		String data;
/*
		int starterIndex = sensingBuffer.indexOf(starter)-78; // S0 嶺뚢돦堉싮뇡�꼻�삕�땻占� �윜諭꾩삕 �뜝�럩�쓧 �뜝�럩�겱�뜝�럩逾졾슖�돦裕뉓땻源띿삕�땻占� �뜝�럥�몥�뜝�럩逾졾뜝�럡�댉濚밸Ŧ�뒩�뜝占�
		int finisherIndex = sensingBuffer.indexOf(finisher, starterIndex+80) + 100; // S4 嶺뚢돦�윪�뇡�꼻�삕�땻占� �뜝�럥�돵�뜝�럥堉� �뜝�럥由띶뜝�럩�졑�뜝�럡�돺�뜝�럡�맋�뜝�럥�몥�뜝�럩逾졾뜝�럡�댉濚밸Ŧ�뒩�뜝占�
*/ // �뛾�렮維싧젆占� 占쎈닱筌뤾쑴�겱�뜝�럩肉녑뜝�럩紐든춯濡녹삕 �뜝�럥�뱺�뜝�럩�몠�뜝�럡�뀏 �뇦猿됲뜑占쎈さ�슖�댙�삕 �뜝�럩援쇿뜝�럡留믣뜝�럥彛�.
...
//		System.out.println(i + "-" + (numOfSensingPoints-1) + " [" + sensorPos + " : " + dataBody.indexOf(" ",sensorPos+1) + "] " + data + " " + SensorValue[numOfSensingPoints-1]);
		sensorPos = sensorPos + data.length() + 1;
///////////////////////////////////////////////////////////////////////// Count	- 濾곌쑬占썼땻類쒕뭄�뜝占� �뜝�럩�쐩 �솻洹⑥삕�뇦猿볦삕 v1114 	
/*///////////////////////////////////////////////////////////////////////// Count
...
//		System.out.println((numOfSensingPoints-1) + "Count : " + SensorValue[numOfSensingPoints-1]);
	...
	{
		//占쎈쐻占쎈윥占쎈ぅ占쎈쐻占쎈윪�얠±�쐻占쎈윞占쎈뙃?占쎈쐻占쎈짗占쎌굲占쎈쐻�뜝占�? 占쎈쐻占쎈윪�억옙占쎈쐻占쎈윥占쎄퐨 占쎈쐻占쎈윥占쎄퐨占쎈쐻占쎈윪占쎌쓨占쎈쐻占쎈윪�얠±�쐻占쎈윥占쎈군 占쎈쐻占쎈짗占쎌굲占쎈쐻占쎈윪占쎄텑占쎈쐻占쎈윥�뵳�뜴�쐻占쎈윥�젆占�.
